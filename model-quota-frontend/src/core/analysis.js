...
/// custom 自定义指令（可空）；profileId/model 指定分析用模型（空 = 当前对话模型）；
/// onEvent(ev) 收 PipelineEvent（ev.type / ev.data）。
export function analyzeFiles(paths, { save = false, custom = '', profileId = '', model = '', onEvent } = {}) {
  const core = tauriCore();
