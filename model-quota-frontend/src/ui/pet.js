...
// （按命中部位触发表情/动作：Head → 表情，Body → Tap 动作）。
// 底部常驻紧凑输入条，可直接与桌宠对话（流式气泡显示回复，会话历史与主窗「对话」页共享）。
//
// 依赖加载顺序是硬约束：pixi-live2d-display 的 cubism4 入口在模块求值时就检查
// window.Live2DCubismCore，缺失即抛错——因此 Cubism Core（版权原因不入 npm，作为
// 静态资源）必须先于库加载，pixi 系模块全部走动态 import（也让 main/mini/ball
// 窗口不必背上 pixi 的体积）。
...
const CORE_SCRIPT = '/assets/lib/live2dcubismcore.min.js';
const DEFAULT_MODEL = '/assets/live2d/haru/haru_greeter_t03.model3.json';
/// 输入条高度（人物底部锚点上移量，避免腿被输入条挡住）
const INPUT_BAR_H = 44;
/// 气泡在回复完成后停留的时长
const BUBBLE_LINGER_MS = 8000;

// 单例加载 Cubism Core（全局 window.Live2DCubismCore）
function loadCubismCore() {
  if (!loadCubismCore.promise) {
    loadCubismCore.promise = new Promise((resolve, reject) => {
      if (globalThis.Live2DCubismCore) return resolve();
      const s = document.createElement('script');
      s.src = CORE_SCRIPT;
      s.onload = () => resolve();
      s.onerror = () => reject(new Error(`Cubism Core 加载失败：${CORE_SCRIPT}`));
      document.head.appendChild(s);
    });
  }
  return loadCubismCore.promise;
}
...
export async function renderPet({ root, repo }) {
  ...
  async function petSend(text) {
    ...
    function finish(text) {
      ...
      // 说话时来个小动作
      modelRef?.motion('Tap', undefined, 3);
    }
  }
  ...
  try {
    ...
    model.motion('Idle', undefined, 3);

    // 原地单击互动：按命中部位触发（Head → 随机表情；Body → Tap 动作组）。
